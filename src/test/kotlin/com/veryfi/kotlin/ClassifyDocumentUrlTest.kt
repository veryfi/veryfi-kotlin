...
import com.veryfi.kotlin.classify.classifyDocumentUrl
import com.veryfi.kotlin.classify.classifyDocumentUrlAsync
...
@SpringBootTest
class ClassifyDocumentUrlTest: ClientTest() {

    @Test
    @Throws(IOException::class, InterruptedException::class)
    fun classifyDocumentUrlTest() {
        if (mockResponses) {
        ...
        val jsonResponse = client.classifyDocumentUrl(
    ...
    @Test
    @Throws(IOException::class, InterruptedException::class)
    fun classifyDocumentUrlTestWithFileUrls() {
        if (mockResponses) {
            val httpClient = mock(HttpClient::class.java)
            client.setHttpClient(httpClient)
            val fileStream = ClassLoader.getSystemResourceAsStream("classify.json")!!
    ...
    fun classifyDocumentUrlAsyncTest() {
        if (mockResponses) {
            val httpClient = mock(HttpClient::class.java)
            client.setHttpClient(httpClient)
            val fileStream = ClassLoader.getSystemResourceAsStream("classify.json")!!
        ...
        val jsonResponseFuture = client.classifyDocumentUrlAsync(
            "https://cdn.veryfi.com/receipts/92233902-c94a-491d-a4f9-0d61f9407cd2.pdf",
            null,
            parameters = null
        )

...
import com.veryfi.kotlin.documents.deleteDocumentLineItems
import com.veryfi.kotlin.documents.deleteDocumentLineItemsAsync
...
@SpringBootTest
class DeleteDocumentLineItemsTest: ClientTest() {

    @Test
    @Throws(IOException::class, InterruptedException::class)
    fun deleteDocumentLineItemsTest() {
        if (mockResponses) {
            val httpClient = mock(HttpClient::class.java)
            client.setHttpClient(httpClient)
            val fileStream = ClassLoader.getSystemResourceAsStream("deleteDocumentLineItems.json")!!
        ...
        val jsonResponse = client.deleteDocumentLineItems("63480993")
        val document = JSONObject(jsonResponse)
        Assertions.assertTrue(document.getString("status").lowercase() == "ok")
    }

    @Test
    @Throws(ExecutionException::class, InterruptedException::class, IOException::class)
    fun deleteDocumentLineItemsAsyncTest() {
        if (mockResponses) {
            val httpClient = mock(HttpClient::class.java)
            client.setHttpClient(httpClient)
            val fileStream = ClassLoader.getSystemResourceAsStream("deleteDocumentLineItems.json")!!
        ...
        val jsonResponseFuture = client.deleteDocumentLineItemsAsync("63480993")

...
import com.veryfi.kotlin.bankstatements.getBankStatements
import com.veryfi.kotlin.bankstatements.getBankStatementsAsync
...
@SpringBootTest
class GetBankStatementsTest: ClientTest() {

    @Test
    @Throws(IOException::class, InterruptedException::class)
    fun getBankStatementsTest() {
        if (mockResponses) {
            val httpClient = mock(HttpClient::class.java)
            client.setHttpClient(httpClient)
            val fileStream = ClassLoader.getSystemResourceAsStream("getBankStatements.json")!!
    ...
    @Test
    @Throws(IOException::class, InterruptedException::class)
    fun getBankStatementsTestWithException() {
        val httpClient = mock(HttpClient::class.java)
        client.setHttpClient(httpClient)
        `when`(
            httpClient.send(
    ...
    fun getBankStatementsAsyncTest() {
        if (mockResponses) {
            val httpClient = mock(HttpClient::class.java)
            client.setHttpClient(httpClient)
            val fileStream = ClassLoader.getSystemResourceAsStream("getBankStatements.json")!!
        ...
        val jsonResponseFuture = client.getBankStatementsAsync()

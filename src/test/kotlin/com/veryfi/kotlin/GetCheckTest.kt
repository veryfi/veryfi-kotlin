...
import com.veryfi.kotlin.checks.getCheck
import com.veryfi.kotlin.checks.getCheckAsync
...
@SpringBootTest
class GetCheckTest: ClientTest() {

    @Test
    @Throws(IOException::class, InterruptedException::class)
    fun getCheckTest() {
        val documentId = "4662680" // Change to your document Id
        if (mockResponses) {
        ...
        val jsonResponse = client.getCheck(documentId)
        val document = JSONObject(jsonResponse)
        Assertions.assertEquals(document.getJSONObject("data").getInt("id"), documentId.toInt())
    }

    @Test
    @Throws(ExecutionException::class, InterruptedException::class, IOException::class)
    fun getCheckAsyncTest() {
        val documentId = "4662680" // Change to your document Id
        if (mockResponses) {
            val httpClient = mock(HttpClient::class.java)
            client.setHttpClient(httpClient)
        ...
        val jsonResponseFuture = client.getCheckAsync(documentId)

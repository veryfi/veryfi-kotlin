...
import com.veryfi.kotlin.split.getSplitDocuments
import com.veryfi.kotlin.split.getSplitDocumentsAsync
...
@SpringBootTest
class GetSplitDocumentsTest: ClientTest() {

    @Test
    @Throws(IOException::class, InterruptedException::class)
    fun getSplitDocumentsTest() {
        if (mockResponses) {
            val httpClient = mock(HttpClient::class.java)
            client.setHttpClient(httpClient)
            val fileStream = ClassLoader.getSystemResourceAsStream("getSplits.json")!!
    ...
    @Test
    @Throws(IOException::class, InterruptedException::class)
    fun getSplitDocumentsTestWithException() {
        val httpClient = mock(HttpClient::class.java)
        client.setHttpClient(httpClient)
        `when`(
            httpClient.send(
    ...
    fun getSplitDocumentAsyncTest() {
        ...
        val jsonResponseFuture = client.getSplitDocumentsAsync(1)

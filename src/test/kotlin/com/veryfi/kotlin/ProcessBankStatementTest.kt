...
import com.veryfi.kotlin.bankstatements.processBankStatement
import com.veryfi.kotlin.bankstatements.processBankStatementAsync
...
@SpringBootTest
class ProcessBankStatementTest: ClientTest() {

    @Test
    @Throws(IOException::class, InterruptedException::class)
    fun processBankStatementTest() {
        if (mockResponses) {
        ...
        val path = ClassLoader.getSystemResource("bankstatement.pdf").path
        val jsonResponse = client.processBankStatement(path, null)
    ...
    fun processBankStatementAsyncTest() {
        if (mockResponses) {
            val httpClient = mock(HttpClient::class.java)
            client.setHttpClient(httpClient)
            val fileStream = ClassLoader.getSystemResourceAsStream("processBankStatement.json")!!
        ...
        val path = ClassLoader.getSystemResource("bankstatement.pdf").path
        val jsonResponseFuture = client.processBankStatementAsync(path, null)

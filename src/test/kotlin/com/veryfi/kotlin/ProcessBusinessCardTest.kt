...
@SpringBootTest
class ProcessBusinessCardTest: ClientTest() {

    @Test
    @Throws(IOException::class, InterruptedException::class)
    fun processBusinessCardTest() {
        if (mockResponses) {
        ...
        val path = ClassLoader.getSystemResource("business_card.jpg").path
        val jsonResponse = client.processBusinessCard(path, null)
    ...
    fun processBusinessCardAsyncTest() {
        if (mockResponses) {
            val httpClient = mock(HttpClient::class.java)
            client.setHttpClient(httpClient)
            val fileStream = ClassLoader.getSystemResourceAsStream("processBusinessCard.json")!!
        ...
        val path = ClassLoader.getSystemResource("business_card.jpg").path
        val jsonResponseFuture = client.processBusinessCardAsync(path, null)

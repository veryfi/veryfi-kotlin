...
import com.veryfi.kotlin.documents.replaceTags
import com.veryfi.kotlin.documents.replaceTagsAsync
...
@SpringBootTest
class ReplaceTagsTest: ClientTest() {
    ...
    fun replaceTagsTest() {
        if (mockResponses) {
            ...
            val fileStream = ClassLoader.getSystemResourceAsStream("replaceTags.json")!!
        ...
        val jsonResponse = client.replaceTags("253736946", listOf("replace_tag"))
        val document = JSONObject(jsonResponse)
        Assertions.assertTrue(document.getJSONArray("tags").count() >= 1)
    }

    @Test
    @Throws(ExecutionException::class, InterruptedException::class, IOException::class)
    fun replaceTagsAsyncTest() {
        if (mockResponses) {
            val httpClient = mock(HttpClient::class.java)
            client.setHttpClient(httpClient)
            val fileStream = ClassLoader.getSystemResourceAsStream("replaceTags.json")!!
        ...
        val jsonResponseFuture = client.replaceTagsAsync("253736946", listOf("replace_tag"))
